namespace TheSlum.GameEngine
{
    ...
    public class Engine
    {
        ...
        {
            string className = inputParams[1];
            object[] param = new object[4];
            param[0] = inputParams[2]; //character ID
            param[1] = int.Parse(inputParams[3]); // character x
            param[2] = int.Parse(inputParams[4]); // character y
            param[3] = (Team)Enum.Parse(typeof(Team), inputParams[5]); // character team
            Character character = (Character) GetInstance("TheSlum.Characters."+className, param);
        ...
        protected void AddItem(string[] inputParams)
        {
            ...
            object[] param = new object[1] { itemId };
            Item item = (Item) GetInstance("TheSlum.Items."+itemClass,param);
            this.characterList.First(x => x.Id == characterId).AddToInventory(item);
        }

        private object GetInstance(string strFullyQualifiedName, object[] inputParams)
        {
            Type t = Type.GetType(strFullyQualifiedName);
            return Activator.CreateInstance(t, inputParams);
        }
